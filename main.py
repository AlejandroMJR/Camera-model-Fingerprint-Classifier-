...
from utils.params import log_dir, models_dir
...
def main():
    ...
    parser.add_argument('--model', help='Model name', type=str, default='ResNet18') # EfficientNetB0, ResNet18
    #parser.add_argument('--db', help='Database name', type=str, default='BasicLoader')
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--min_lr', type=float, default=1e-8)
    parser.add_argument('--epochs', type=int, default=500)
    parser.add_argument('--es_patience', type=int, default=10, help='Patience for stopping the training if no improvement'
    ...
    parser.add_argument('--aug', help='Augmentation to perform with probability `aug_prob`', nargs='+') #,
                        ...
                        #          'downscale', 'hsv', 'jpeg'])
    parser.add_argument('--aug_prob', help='Probability of augmentation', type=float, default=0)
    ...
    # Checkpoint paths
    train_tag = make_train_tag(network_class, lr, aug_list, aug_prob, batch_size, num_classes)
    ...
    for epoch in range(init_epoch, epochs):
        ...
        for batch_data in tqdm(train_dl, desc='Training epoch {}'.format(epoch), leave=False, total=len(train_dl)):
            ...
            # Forward pass
            outputs, batch_loss = batch_forward(net, device, criterion, batch_img, batch_label)
        ...
        for batch_data in tqdm(val_dl, desc='Validating epoch {}'.format(epoch), leave=False, total=len(val_dl)):
            ...
            _, y_pred = torch.max(outputs, dim=1)
            val_acc += torch.sum(y_pred == batch_label.to(device))

            # Iteration logging
            tb.add_scalar('validation/it-loss', batch_loss.item(), val_tot_it)

        number_of_datapoints = len(val_dl) * batch_size
        final_val_acc = val_acc / number_of_datapoints
        print('\nValidation loss epoch {}: {:.4f}'.format(epoch, val_loss / val_num))
        print('\nValidation accuracy epoch {}: {:.4f}'.format(epoch, final_val_acc))
        # Logging
        ...
        tb.add_scalar('val_accuracy/epoch', final_val_acc, epoch )
        tb.add_scalar('train_accuracy/epoch', final_train_acc, epoch)
        tb.flush()
