...
class BasicLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tranform_rgb2gray_aug = A.Compose(rgb2gray_aug)
        ...
        for patch in patches:
            ...
            if self.aug_transformers is None:
                transf_rgb2gray = tranform_rgb2gray_aug(image=patch)['image']
                transform = [
                    A.Normalize(mean=np.mean(transf_rgb2gray / 255.), std=np.std(transf_rgb2gray / 255.) + torch.
                                finfo(torch.float32).eps, ),
                    A.pytorch.transforms.ToTensorV2(),
                ]
                trans = A.Compose(transform)
                trans_patch = trans(image=transf_rgb2gray)['image']
            else:
                tranform_aug = A.Compose(self.aug_transformers + rgb2gray_aug)
                transf_aug = tranform_aug(image=patch)['image']
                transform = [
                    A.Normalize(mean=np.mean(transf_aug / 255.), std=np.std(transf_aug / 255.) + torch.finfo(
                        torch.float32).eps, ),
                    A.pytorch.transforms.ToTensorV2(),
                ]
                trans = A.Compose(transform)
